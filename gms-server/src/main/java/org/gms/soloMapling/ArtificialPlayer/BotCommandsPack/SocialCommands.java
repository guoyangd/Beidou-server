...
import static org.gms.soloMapling.server.SoloMaplingUtilities.generateRandomNumber;
...
public class SocialCommands {
    ...
    public static void displayPlayerChatCommands(Character chr, ChatCommandResult msg1) {
        ...
        int duration = 25;
        chr.getClient().sendPacket(PacketCreator.sendHintTimed(msg1.getMessage(), maxWidth, duration));
        chr.getClient().sendPacket(PacketCreator.enableActions());
    }

    public static void expirePlayerChatCommands(Character chr) {
        chr.getClient().sendPacket(PacketCreator.sendHintTimed(".", 40, 0));
        chr.getClient().sendPacket(PacketCreator.enableActions());
    }
