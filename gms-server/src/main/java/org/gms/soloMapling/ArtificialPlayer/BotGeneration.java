...
public class BotGeneration {
    ...
    public static Character getConsoleBot() {
        ...
        int botId = 999;
        int baseId = SoloMaplingConstants.GameConstants.BOT_TEMPLATE_CID;
    ...
    public static int createBot(Point pos, MapleMap map, int baseClass, int minLevel, int maxLevel, int forcedJobId) {
        int cid = SoloMaplingConstants.GameConstants.BOT_TEMPLATE_CID;
...
//        final Channel channel = Server.getInstance().getChannel(BotSM.GameConstants.WORLD_SCANIA, BotSM.GameConstants.CHANNEL_1);
        // BeiDou 的 awayFromWorld 默认 true（登录时才置 false），Bot 不走登录流程——
        // 不显式置位会被 cleanupGhostPlayers 当"幽灵玩家"清掉，后续 Bot 一进图就把先前的 Bot 全删了。
        // 只置位不挂组队搜索池：保持上游语义（Bot 经组队生命周期事件自然进搜索池，
        // 队长可用组队搜索找到 Bot 并邀请，右键直接邀请也走 BotPartyQueue）。
        fakechar.setEnteredChannelWorldWithoutPartySearch();
